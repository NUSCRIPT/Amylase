...
```{r, echo=FALSE}
#Create a table with label and p values
p_value_table <- data.frame(Name=c("Alive", "Amylase_250", "Amylase_400", "Amylase_500"), p_values=c(0.01, 0.059, 0.0064, 0.0011))
...
p_value_table <- data.frame(Name=c("All", "Covid_negative"))
p_values=c(0.0014, 0.039)
...
#Generate time-to-event curve for time to extubation for Alive
Alive <- subset(Covid_negative, Mortality == "0")
fit <- survfit(Surv(intubation_duration) ~ group, data=Alive)
print(fit)
p <- ggsurvplot(fit, pval = TRUE, conf.int = TRUE, risk.table = FALSE, risk.table.col = "strata", linetype = "strata", surv.median.line = "hv", ggtheme = theme_bw(), palette = c("Dark2"), xlim=c(0,45), font.main = c(14), font.x = c(10), font.y = c(14), font.tickslab = c(12, "plain"), xlab = "Time to Extubation in Days", ylab= "", font.legend=c(12), title="Discharged from Hospital Alive", break.x.by=5, legend.title="BAL Amylase Group", legend.labs=c("High","Low-Intermediate"), legend.position="bottom")
...
## Cox proportional hazards models for Covid negative cases at different amylase thresholds

```{r, echo = FALSE}
cox <- coxph(Surv(intubation_duration, status) ~ Amylase_250 + edw_adm_age + bmi + norepinephrine_avg + gender + Cardiac_arrest + Mortality, data = Time_to_event2)
summary(cox)

cox <- coxph(Surv(intubation_duration, status) ~ Amylase_500 + edw_adm_age + bmi + norepinephrine_avg + gender + Cardiac_arrest + Mortality, data = Time_to_event2)
summary(cox)

cox <- coxph(Surv(intubation_duration, status) ~ Amylase_400 + edw_adm_age + bmi + norepinephrine_avg + gender + Cardiac_arrest + Mortality, data = Time_to_event2)
summary(cox)
...
## ARDS Phenotypes in Non-Covid Pneumonia
...
## Get non-covid samples from Amylase dataset, still use first BAL from each patient
Amylase <- subset(CDdataset, !is.na(amylase))
Amylase <- Amylase[grep("BAL-00", Amylase$bal_barcode), ]
Amylase <- Amylase %>% mutate(Mortality = case_when(discharge_disposition_name == 'Died' ~ "1"))
Amylase <- Amylase %>% mutate(Mortality = ifelse(is.na(Mortality), 0, Mortality))
...
Create K means clusters and Linegraph

```{r}

fviz_nbclust(data_scaled[1:21], kmeans, method = "wss")
...
fviz_cluster(k2, data = data_scaled[1:21], geom="point")
...
#Create Linegraph of two clusters
linegraph_df <- data_scaled_clusters %>% select(plateau_pressure_avg, respiratory_rate_avg, peep_avg, minute_ventilation_avg, bmi, gender, wbc_avg, heart_rate_avg, platelet_avg, creatinine_avg, bilirubin_avg, albumin_avg, hemoglobin_avg,  temperature_avg, amylase, bicarbonate_avg, sodium_avg,  systolic_blood_pressure_avg, mean_arterial_pressure_avg,  edw_adm_age, po2_fio2_ratio_avg, Cluster)
...
p + theme_bw() + scale_color_brewer(palette = "Set2") + ggtitle("Non Covid Pneumonia Clusters by ARDS Phenotype Variables") + theme(plot.title = element_text(hjust=0.5)) + theme(legend.text = element_text(size=14), axis.title=element_text(size=14), legend.title = element_text(size=14)) + labs(y="Scaled Mean", x="") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=12)) + scale_x_discrete(labels=c("plateau_pressure_avg" = "Plateau Pressure", "respiratory_rate_avg" = "Respiratory Rate", "peep_avg" = "PEEP", "minute_ventilation_avg"="Minute Ventilation", "bmi"="BMI", "gender"="Gender", "wbc_avg"="WBC",  "heart_rate_avg"="Heart Rate", "platelet_avg"="Platelets", "creatinine_avg"="Creatinine", "bilirubin_avg"="Bilirubin", "albumin_avg"="Albumin", "hemoglobin_avg"="Hemoglobin",  "temperature_avg"="Temperature", "amylase"="BAL Fluid Amylase", "bicarbonate_avg"="Bicarbonate", "sodium_avg"="Sodium",  "systolic_blood_pressure_avg"="Systolic Blood Pressure", "mean_arterial_pressure_avg"="MAP",  "edw_adm_age"="Age", "po2_fio2_ratio_avg"="P:F Ratio"))
...
```{r}
## Outcomes by Cluster
tapply(data_scaled_clusters$cumulative_intubation_days, data_scaled_clusters$Cluster, summary)
kruskal.test(cumulative_intubation_days ~ Cluster, data = data_scaled_clusters)

tapply(data_scaled_clusters$total_icu_los_days, data_scaled_clusters$Cluster, summary)
kruskal.test(total_icu_los_days ~ Cluster, data = data_scaled_clusters)

tapply(data_scaled_clusters$hospital_los_days, data_scaled_clusters$Cluster, summary)
kruskal.test(hospital_los_days ~ Cluster, data = data_scaled_clusters)

table(data_scaled_clusters$Mortality, data_scaled_clusters$Cluster)
FE <- data.frame("Death" = c(68, 74), "Alive" = c(125, 93), row.names = c("Cluster1", "Cluster2"))
fisher.test(FE)

## Find Duration of Extubation
data_scaled_extubation <- EDW_basic_endpoints[EDW_basic_endpoints$pt_study_id %in% c(data_scaled_clusters$pt_study_id),]
Extubation_df <- merge(data_scaled_clusters, data_scaled_extubation, by=("pt_study_id"))

Extubation_df2 <- Extubation_df
Extubation_df2$intubation_duration <- time_length(Extubation_df2$First_intub_stop - Extubation_df2$First_intub_start, unit="days")

tapply(Extubation_df2$intubation_duration, Extubation_df2$Cluster, summary)
kruskal.test(intubation_duration ~ Cluster, data = Extubation_df2)
```
